...
class LocalSpeakingAssessmentReport:
    ...
    End-to-end local pipeline (no server needed):
      ...
      1) Download (if URL) + transcribe each audio with Whisper (OpenAI)
      2) Compute metrics:
    ...
    def generate_report(self, pairs: List[ListenRepeatPair], out_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for p in pairs:
                if self.task == "interview":
                    # ------ INTERVIEW MODE: prompt is TEXT (topic); reference audio does NOT exist ------
                    topic_text = (p.prompt_audio or "").strip() or self._topic_from_filename(p.student_audio)

                    # only fetch/transcribe the student audio
                    student_path, e2 = self._to_local(p.student_audio, temp_dir)
                    if e2:
                        errors.append(self._err_obj("-", p.student_audio, "DownloadError", e2))
                        prompt_tx.append((topic_text, 0.0))
                    ...
                    prompt_tx.append((topic_text, 0.0))   # keep topic for relevance scoring
                    student_tx.append((st_text, st_dur))
                    student_durations.append(st_dur)

                    # per-item pause stats
            ...
            # 2a) Repeat accuracy + incorrect segments
            if self.task == "interview":
                # No reference read-back exists in ICNALE
                repeat_accuracy_score = 0
            ...
            if self.task == "interview":
                # No reference audio to compare against; keep neutral availability
                pron_acc_score = self._pronunciation_score([], total_student_words)
            else:
            ...
            # ---- Choose a single accuracy-like value for both Fluency and Overall ----
            acc_like = repeat_accuracy_score if self.task == "listen_repeat" else (
                grammar_score if isinstance(grammar_score, int) else 80
            )
            ...
            if self.task == "listen_repeat":
                fluency = self._build_fluency_section(
                    ...
                    acc=acc_like,  # use chosen accuracy-like value
                ...
                # --- interview ---
                rel_label = self._embedding_relevance(prompt_tx, student_tx)   # topic vs answer
                disc_label = self._gpt4o_discourse(all_student_text)
                wr_level = self._word_repetition_level(all_student_text)
                fluency = self._build_fluency_section(
                    speech_rate=speech_rate,
                    acc=acc_like,  # use grammar-derived/neutral accuracy-like value
                    pause_freq_lvl=pause_freq_lvl,
                    pause_app_lvl=pause_app_lvl
                )
            ...
            report = {
                ...
                "incorrect_segments": incorrect_segments[:50],
                "mispronounced_words": [{"word": w} for w in mispronounced_words],
                "fluency": fluency,
                "pronunciation": pronunciation,
                "grammar": grammar,
            }
            if self.task == "interview":
                # include topics used (one per audio)
                report["topics"] = [q for (q, _) in prompt_tx]
    ...
    def _gpt4o_mispronunciations(self, prompt_audio: str, student_audio: str,
        ...
        def audio_part(b: bytes) -> Dict[str, Any]:
            return {
                "type": "input_audio",
                "input_audio": {"data": base64.b64encode(b).decode("utf-8"), "format": "wav"},
            }

        messages = [
            {
                "role": "system",
                "content": """You are a precise ASR/phonetics judge. Compare the student’s pronunciation to the reference audio + text. Return ONLY valid JSON: {"words": ["word1", ...]} with unique, lowercase mispronounced words. If none, return {"words": []}"""
            },
            {
                "role": "user",
                "content": [
    ...
    @staticmethod
    def _topic_from_filename(path_or_url: str) -> str:
        """Infer ICNALE topic from the filename."""
        name = os.path.basename(urlparse(path_or_url).path).upper()
        if any(k in name for k in ("PTJ","PTJ1","PTJ2")):
            return "part-time employment"
        if any(k in name for k in ("SJT","SMK","SR")):
    ...
    def _transcribe(self, audio_path: str) -> Tuple[str, float, List[Dict[str, float]]]:
        ...
        audio.export(audio_buffer, format="wav")  # Re-encode to WAV
        audio_buffer.seek(0)
        audio_buffer.name = "audio.wav"

        # ASR (always do it when called here, because we need timestamps)
        asr_result = self._openai.audio.transcriptions.create(
            file=audio_buffer,
            model="whisper-1",
...
if __name__ == "__main__":
    ...
    # EITHER: --pairs prompt.wav:student.wav (listen_repeat)
    # OR:     --audios student1.mp3 student2.mp3 ... (interview)
    g = ap.add_mutually_exclusive_group(required=True)
    g.add_argument("--pairs", nargs="+", help="prompt.wav:student.wav (or URLs)")
    g.add_argument("--audios", nargs="+", help="Student audio files/URLs; interview mode only")

    # Optional topic override for interview mode
    ap.add_argument("--topic", default="", help="Topic text to use for all audios (interview mode). If empty, inferred from filename.")
    args = ap.parse_args()

    prs: List[ListenRepeatPair] = []

    if args.audios:
        if args.task != "interview":
            raise SystemExit("--audios can only be used with --task interview")
        topic_text = (args.topic or "").strip()  # may be empty; generate_report will infer per file if empty
        for a in args.audios:
            # In interview mode, prompt_audio carries TEXT (topic), not audio.
            prs.append(ListenRepeatPair(prompt_audio=topic_text, student_audio=a))

    else:
        # listen_repeat path, unchanged
        if args.task != "listen_repeat":
            # allow interview with pairs if you ever want, but it's unusual
            pass
        for p in args.pairs:
            try:
                prompt, student = p.split(":", 1)
            except ValueError:
                raise SystemExit(f"Bad --pairs item: '{p}'. Expected 'prompt:student'.")
